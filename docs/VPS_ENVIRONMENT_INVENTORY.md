...
Added for the direct Google login feature (`feat/google-oauth-direct`) — a parallel login path alongside the existing Manus OAuth flow, gated by a feature flag so it changes nothing for a deployment that doesn't explicitly opt in. See `server/_core/googleOAuth.ts`, `server/_core/googleOidc.ts`, and `server/services/googleIdentityService.ts`. **No real Client ID/Secret value is recorded anywhere in this repo, in this doc, or in any commit — names and behavior only, per this feature's explicit constraints.**

| Variable | Build/Runtime | Secret/Public | Required/Optional | Used by | Description | VPS value source | Verification method |
|---|---|---|---|---|---|---|---|
| `AUTH_PROVIDER` | Runtime | Public (mode flag) | Optional — **defaults to `manus`** for any value other than the exact literal `google` (unset, typo, empty) | `server/_core/env.ts`, `server/_core/googleOAuth.ts` | Selects which server-side login flow's routes are active (`manus` or `google`) — the ONLY thing that turns the new `/api/auth/google/*` routes on; unset or any other value makes them behave as if they don't exist (plain 404) | `manus` in production until an explicit, separate cutover decision; `google` on staging only, once Google Cloud Console's registered redirect URI matches `GOOGLE_OAUTH_REDIRECT_URI` below | Presence/value-only check — never a secret |
| `VITE_AUTH_PROVIDER` | **Build** (client bundle) | Public | Optional — same default-to-`manus` behavior as `AUTH_PROVIDER`, but set independently (the client and server flags are never derived from each other) | `client/src/const.ts`'s `getLoginUrl()`/`resolveLoginUrl()` | Selects which login URL the client's "Sign in" buttons point at | Same value as `AUTH_PROVIDER` for a given environment, kept in sync manually — the two are intentionally independent variables, not one variable read twice, so a deploy that forgets one fails closed to Manus rather than mixing flows | Presence/value-only check |
...
| `GOOGLE_OAUTH_REDIRECT_URI` | Runtime | Public (URL) | Required for the Google flow to function | `server/_core/env.ts`, `server/_core/googleOAuth.ts` (authorization request), `server/_core/googleOidc.ts` (token exchange) | The exact callback URL registered in Google Cloud Console (`https://<domain>/api/auth/google/callback`) — **read verbatim from this environment variable, never derived from the request's `Host` or `X-Forwarded-Host` header** (both are client-influenced; Google's token endpoint requires an exact match to what was registered) | Staging: `https://staging.ipenovel.com/api/auth/google/callback` (already registered in Google Cloud Console per this feature's task). Production: set only as part of a later, separate cutover decision. | Presence + URL-parses check; must byte-for-byte match Google Cloud Console's registered redirect URI |
