...
//   3. Opens its OWN connection built directly from TEST_DATABASE_URL, then
//      runs a live "SELECT DATABASE()" check
...
export async function runTestDbMigration(): Promise<void> {
  ...
  // anything else, and never proceeds.
  ...
  try {
    connection = mysql.createConnection(testUrl!);
  } catch (error) {
