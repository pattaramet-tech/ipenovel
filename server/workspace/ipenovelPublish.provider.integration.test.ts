...
describe.sequential("IpeNovel Workspace external publish adapter", () => {
  it("publishes only the exact destination episode and reconciles the deterministic receipt without duplicate delivery", async () => {
    ...
    try {
      ...
      const [reconciledNovel] = await db
        .select()
        .from(novels)
        .where(eq(novels.id, novel.id));
  ...
  it("blocks publish before parent or episode visibility when contamination is unsafe to auto-remediate", async () => {
    if (!process.env.TEST_DATABASE_URL) return;
    assertSafeTestDatabaseUrl(process.env.TEST_DATABASE_URL);
    const db = getTestDb();
    const owner = await createTestUser({ role: "admin" });
    ...
    try {
      ...
      const [afterNovel] = await db
        .select()
        .from(novels)
        .where(eq(novels.id, novel.id));
      expect(afterNovel.publicationStatus).toBe("archived");
    } finally {
      await db
        .delete(workspaceWorkspaces)
        .where(eq(workspaceWorkspaces.id, workspace.workspaceId));
