...
describe("NQA M12 structure isolation", () => {
  ...
  it("enforces loopback-only structure provider endpoints", () => {
    const source = fs.readFileSync(path.join(DIR, "provider.ts"), "utf8");

    expect(source).toContain('"127.0.0.1"');
    expect(source).toContain('"localhost"');
    expect(source).toContain('"::1"');
    expect(source).toContain(
      "Local structure endpoint must use a loopback hostname."
    );
  });
});
