...
export const appRouter = router({
  ...
  checkout: router({
    ...
    // Preview cart without creating order
    preview: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        try {
          // Calculate totals without creating order
          let subtotal = "0";
          for (const item of cartItems) {
            subtotal = (parseFloat(subtotal) + parseFloat(item.price.toString())).toFixed(2);
          }
          ...
          if (input.couponCode) {
            ...
            if (coupon) {
              if (coupon.discountType === "percentage") {
                discountAmount = (parseFloat(subtotal) * parseFloat(coupon.discountValue.toString()) / 100).toFixed(2);
          ...
          if (input.pointsToRedeem) {
            const pointsValue = parseFloat(input.pointsToRedeem);
            pointsDiscountAmount = pointsValue.toFixed(2);
          }
    ...
    submitPayment: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          const message = error?.message || "Failed to submit payment";
          throw new TRPCError({ code: "BAD_REQUEST", message });
        }
      }),
